...
class PhoneField(Field):
    def __init__(self, value):
        super().__init__(value)  # Приватне поле для зберігання номера телефону.
        self.value = None  # Встановлення значення через setter.
...
class Record:
    ...
    def add_phone(self, phone_number):
        self.phones.append(Name(phone_number))  # Додавання номера телефону.
    ...
    def __str__(self):
        return f"Contact name: {self.name.value}, phones: {'; '.join(p.value for p in self.phones)}"  # Повернення
        # рядка з інформацією про контакт.
...
class AddressBook(UserDict):
    ...
    def get_upcoming_birthdays(self, days=7) -> list:
        ...
        for user in self.data.values():
            ...
            if birthday_this_year.month == 2 and birthday_this_year.day == 29 and not calendar.isleap(today.year):
                birthday_this_year = datetime(today.year, 3,
                                              1).data()  # Переносимо на 1 березня , якщо рік  не високосний
